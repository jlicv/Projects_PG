...
n_tick = log_rtn_series.resample('45min').count()

vol_std = log_rtn_series.resample('45min').std()*pow(365*32,0.5)
vol_std = vol_std.fillna(0)
#vol_std.to_csv("vol-tick-45minutes.csv")
#You have to bind it with a 0
#一开始选择的是0到31行，所以要加多一行
cumsum_n_tick = np.cumsum(n_tick)
extrarow_cumsum = cumsum_n_tick[0:1]
extrarow_cumsum = extrarow_cumsum.shift(freq='-45min')
extrarow_cumsum[0] = 0
...
for i in range(0,len(cumsum_n_tick)-1):
    ...
    if(n!=0):
        ...
        Vol_TwoScale = Vol_slice.mean()*pow(365*32,0.5) - nbar_initial/n*vol_std[i]
    else:
        Vol_TwoScale = 0
...
vol_df.plot()
vol_df.to_csv('vol-twoestimates-45min.csv')