...
function[pi_estimate,err,time] = CCVS(T,U,Npath,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
pi = 100/(2*K)*(corridorvar-1/N*K^2*sum(w_2_inside,2));
...
output_array = {'Time consumed is','Number of path(Antithetic)simulated is','The estimate of pi_T is'...
    ,'Estimated std. of pi_T''s estimate is';time,Npath,pi_estimate,err};
string = sprintf('%s:%d;\n',output_array{:});
fprintf(fID,string);
fclose(fID);
