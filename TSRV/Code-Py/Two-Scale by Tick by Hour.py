...
Var_alltick = df.groupby(df['time'].map(lambda x:x.day*24+x.hour)).std()

#制作第一层Index，按天数划分
#日期中的天数
index_day = df['time'].map(lambda x:x.day*24+x.hour).values
...
for i in range(TwoScale_Parameters.shape[0]):    
    ...
    index = pd.MultiIndex.from_arrays([index_day,index_K],names=['hour','K'])
    #根据这个Index计算各组的方差
    Var_slice = df.groupby(index).std()
    #把Index重新制作成MultiIndex
    Var_slice.index = pd.MultiIndex.from_tuples(Var_slice.index,names=['hour','K'])
    #这下子就可以算日内RV的均值了！
    #发现用Python解锁了做透视表的技能，开心~！
    Var_pool = Var_slice.mean(level='hour')
