...
Estm = pd.read_csv('Daily_TwoScale_Minute.csv')
Estm = Estm.fillna(0)
#Don't simply change the values of column names, use rename instead
Estm = Estm.rename(columns={Estm.columns.values[0]:'Hour'})
Estm_xs = Estm['Hour'].unique()
Estm_zs = Estm['K'].unique()[1]
Estm_verts = []
fig = plt.figure()
...
for z in Estm_zs:
    Estm_ys =Estm.groupby('K').get_group(z)['log_rtn']*pow(525600,0.5)
