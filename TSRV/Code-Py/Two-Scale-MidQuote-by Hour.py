...
n_tick = log_rtn.resample('1H').count()
#对应Zhang,MykLand(2005)里面的Fifth Estimator，直接根据Tick的数据，求RV。
var_all = square_rtn.resample('1H').sum()
#对应Zhang,Mykland(2005)里面的Fourth Estimator，稀疏采样，求RV。
#::40，意思是每隔40个Tick抽一个点。
#你可以不年化先。比较完以后再一起年化。
var_45_sparse = square_rtn.resample('1H').apply(lambda x:sum(x[::20]))
...
extrarow_cumsum = extrarow_cumsum.shift(freq='-1H')
extrarow_cumsum[0] = 0
...
var_df.plot()
var_df_ann = var_df * 365 * 24
var_df_ann.plot()
var_all_ann = var_all*365*24
var_all_ann.plot()
var_df.to_csv('var-twoestimates-1H.csv')