# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 17:25:39 2017
...
#按照天数统计笔数
n_Tick_Count_Minute = df.groupby(df['time'].map(lambda x:x.day*1440+x.hour*60+x.minute)).agg(['count'])
#能不能再按照每5分钟切分呢？
#并不需要，实际上笔数的分布，按天看，并不均匀。
#制作一个数组，存放各个K和n_bar的数值。
n = pd.DataFrame.mean(n_Tick_Count_Minute).loc['time'].values
...
Var_alltick = df.groupby(df['time'].map(lambda x:x.day*1440+x.hour*60+x.minute)).std()

#制作第一层Index，按天数划分
#日期中的天数
index_day = df['time'].map(lambda x:x.day*1440+x.hour*60+x.minute).values
...
for i in range(TwoScale_Parameters.shape[0]):    
    ...
    #制作一个两层的Index，第一层是天，第二层是Bin.
    index = pd.MultiIndex.from_arrays([index_day,index_K],names=['minute','K'])
    #根据这个Index计算各组的方差
    Var_slice = df.groupby(index).std()
    #把Index重新制作成MultiIndex
    Var_slice.index = pd.MultiIndex.from_tuples(Var_slice.index,names=['minute','K'])
    #这下子就可以算日内RV的均值了！
    #发现用Python解锁了做透视表的技能，开心~！
    Var_pool = Var_slice.mean(level='minute')
    #做一点点变换，就得到Two Scale RV的按日估计了。
    Var_TwoScale = Var_pool - TwoScale_Parameters.loc[i,'nbar']/n*Var_alltick
...
Estm.to_csv('Daily_TwoScale_Minute.csv')