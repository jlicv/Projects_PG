function [ price,time ] = VA(T,K,I,J,r,rg,alpha,gamma,sigma)
tic
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%By: Juncheng Li
%Date:2016/04/24
...
for i = I+1:-1:1
    for j = J:-1:j0
        ...
        jfloor = floor(jwave);
        if jfloor + 1 > J
            VExtra = payoff(i,J) + (jwave - J)*(payoff(i,J)-payoff(i,J-1))/dp; %Extrapolation
            V(T,1,i,j) = VExtra;
        else
...
for t =T:-1:2
    for k = 1:K+1
        if k == K+1 
            %Incorporate the Jump Condition.And also the Early Exercise Decision.
            for i = I+1:-1:1
                for j = J:-1:j0 %You might need to consider the Pintrinsic after t.
                    V(t,k,i,j)=max(j*dp,V(t,k,i,j));
            ...
            for i = I+1:-1:1
                for j = J:-1:j0
                    ...
                    jfloor = floor(jwave);
                    if jfloor + 1 > J
                        VExtra = V(t,k,i,J) + (jwave - J)*(V(t,k,i,J)-V(t,k,i,J-1))/dp; %Extrapolation
                        V(t-1,1,i,j) = VExtra; 
                    else
